...
- [x] 增加活动日志保留天数和最大数据库体积清理任务。
- [x] 增加 SQLite WAL 模式和 busy timeout 的评估与测试。
- [ ] 增加数据库备份、恢复和完整性检查文档。
...
- [x] 支持管理员在 Bot 中启用/停用用户的 115 配置。
- [x] 支持用户查看和选择 115 目标目录。
- [x] 支持管理员查看当前 `unknown` 解锁记录并进行人工处理。
- [ ] 支持配置多个 HDHive 账号或按用户隔离 HDHive 身份。
- [x] 支持 SQLite 数据导出为脱敏 JSON。
